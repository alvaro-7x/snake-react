...
export class GraficosService
{
  ...
  private heightDetalles = 50;
  private d: number = ResolucionJuego.d;
  private distancia = 38;
  ...
  dibujarGameOver (ctx: CanvasRenderingContext2D)
  {
    ...
    const snake = this.imagenesService.snake;
  ...
  dibujarTextoDetalles (ctx: CanvasRenderingContext2D, puntoX: number, puntoY: number, datoJuego: DatoDetalleJuego)
  {
    ...
    const tmpDato = String(dato).padStart(longPadStart, '0');
  ...
  dibujarVidas (ctx: CanvasRenderingContext2D, snakeDetalles: HTMLImageElement, vidas:number)
  {
    ...
    const sx = 0;
    const sy = 0;
    const sWidth = snakeDetalles.width;
    const sHeight = snakeDetalles.height;

    const dx = newPuntoX;
    const dy = newPuntoY;
    const dWidth = 150;
    const dHeight = this.d;
  ...
  dibujarCruceta (ctx: CanvasRenderingContext2D, x: number, y: number, color:string, direccion:number|null = null)
  {
    ...
    this.dibujarPuntoTriangularControles(ctx, x, y, '#b1b0b0', Movimientos.UP, 9, direccion == Movimientos.UP);
    this.dibujarPuntoTriangularControles(ctx, x, y + 2, '#b1b0b0', Movimientos.DOWN, 9, direccion == Movimientos.DOWN);

    this.dibujarPuntoCircularControles(ctx, x, y + 1, '#32383e', true);

    this.dibujarPuntoTriangularControles(ctx, x - 1, y + 1, '#b1b0b0', Movimientos.LEFT, 9, direccion == Movimientos.LEFT);
    this.dibujarPuntoTriangularControles(ctx, x + 1, y + 1, '#b1b0b0', Movimientos.RIGHT, 9, direccion == Movimientos.RIGHT);
  }
  ...
  dibujarPuntoControles (ctx: CanvasRenderingContext2D, puntoX: number, puntoY: number, color: string, colorNormal = false)
  {
    ...
    const grd = ctx.createLinearGradient(newPuntoX - 25, newPuntoY - 25, newPuntoX + 10, newPuntoY + 10);
  ...
  dibujarPuntoCircularControles (ctx: CanvasRenderingContext2D, puntoX: number, puntoY: number, color:string, gradiente = false)
  {
    const newPuntoX = (this.distancia * (puntoX - 1));
    const newPuntoY = (this.distancia * (puntoY - 1));
    ...
    const radio = 50;
    const grd = ctx.createRadialGradient(newPuntoX + (this.distancia / 2), newPuntoY + (this.distancia / 2), radio, newPuntoX - (this.distancia / 2) - 7, newPuntoY - (this.distancia / 2) - 7, radio);
  ...
  dibujarBotonAccion (ctx: CanvasRenderingContext2D, puntoX: number, puntoY: number, color:string, texto:string, radio:number, init = false)
  {
    const newPuntoX = (this.distancia * (puntoX - 1));
    const newPuntoY = (this.distancia * (puntoY - 1));
    ...
    const grd = ctx.createRadialGradient(newPuntoX, newPuntoY, radio, newPuntoX, newPuntoY - 7, radio);
    grd.addColorStop(0.5, '#212529');
    grd.addColorStop(1, color);
  ...
  dibujarBotonOpcion (ctx:CanvasRenderingContext2D, puntoX: number, puntoY: number, color:string, texto:string, presionado:null|boolean = false)
  {
    if (presionado == true)
    {
      this.dibujarBotonOpcionBorde(ctx, puntoX, puntoY, '#b1b0b0', 4);
    }
    else
    {
    ...
    if (presionado == null)
    {
      this.dibujarTextoControles(ctx, puntoX + 0.5, puntoY + 0.6, 'black', texto, 11);
    }
  }
  ...
  dibujarTextoControles (ctx: CanvasRenderingContext2D, puntoX: number, puntoY: number, color:string, texto:string, size:number|null = null)
  {
    ...
    const fontSize = size || this.distancia;
  ...
  dibujarPuntoTriangularControles (ctx: CanvasRenderingContext2D, puntoX: number, puntoY: number, color: string, direccion: number, porcentaje = 0, relleno = false)
  {
    const newPuntoX = (this.distancia * (puntoX - 1));
    const newPuntoY = (this.distancia * (puntoY - 1));
